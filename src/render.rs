...
/// Normalize KaTeX-rendered HTML so it's safe to inject back into markdown.
///
/// - Replaces literal `_` in HTML text nodes with `&#95;` so `pulldown-cmark` won't treat them as
///   emphasis markers (which can break the KaTeX HTML).
/// - Replaces newlines with spaces, matching KaTeX render normalization used elsewhere.
///
...
fn normalize_katex_html_for_markdown<'a>(
    ...
    for (index, byte) in rendered_html.bytes().enumerate() {
        match byte {
            b'<' => in_tag = true,
            b'>' => in_tag = false,
            b'_' if !in_tag => {
                if !changed {
                    scratch.clear();
                    scratch.reserve(rendered_html.len() + 16);
                    changed = true;
                }
                scratch.push_str(&rendered_html[cursor..index]);
                scratch.push_str("&#95;");
                cursor = index + 1;
            }
            b'\n' => {
    ...
    scratch.push_str(&rendered_html[cursor..]);
    scratch.as_str()
}
...
pub fn render(item: &str, opts: Opts, extra_opts: ExtraOpts, display: bool) -> String {
    ...
    match katex::render_with_opts(normalized_tex.as_ref(), opts) {
        Ok(rendered) => {
            let rendered = normalize_katex_html_for_markdown(&rendered, &mut scratch);
